...
class Chapter1_SimplifiedHash extends React.Component {
    ...
    render() {
        ...
        return <div className="chapter1">
              ...
              <p> Second, this page discusses <code>dict</code> as it is implemented <a href="http://python.org/">CPython</a> &mdash; the "default" and most common implementation of python (if you are not sure what implementation you use, it is almost certainly CPython). Some other implementations are PyPy, Jython and IronPython. The way dict works in each of the implementation may be similar to CPython (in case of PyPy) or very different (in case of Jython). </p>
              ...
              <p> Appending to a list is fast. Getting an element by index is fast too. But there is no order whatsoever, so searching for a specific element is slow. We may get lucky and find an element only in a few iterations if it is near the beginning of the list. But if it is not here at all, we'll have to scan over the whole array. </p>
              ...
              <p> Sure, we only have a few elements here, so scanning over them is no big deal. But what if we have a million of distinct numbers? If a number is missing, verifying this requires looking through the whole million of numbers. </p>
              <p> What we can do is organizing our data in a quite different way. Here is how. Let's create a new list. This list will be a list of slots where we'll put numbers from the original list. And we'll use the number itself to compute an index of a slot for the number. The super simple way is just take the slot <code> number % len(the_list) </code> and put our number there. Would this approach work? Not quite. For example, two numbers (TODO: compute it) would be put in the same slot. Such situtations are called <em>collisions</em>.</p>
