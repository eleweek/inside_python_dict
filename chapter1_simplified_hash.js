...
class Chapter1_SimplifiedHash extends React.Component {
    ...
    render() {
        ...
        return <div className="chapter1">
              ...
              <p> Second, this page discusses <code>dict</code> as it is implemented <a href="http://python.org/">CPython</a> &mdash; the "default" and most common implementation of python (if you are not sure what implementation you use, it is almost certainly CPython). Some other implementations are <a href="https://pypy.org/">PyPy</a>, <a href="http://www.jython.org/">Jython</a> and <a href="http://ironpython.net/">IronPython</a>. The way dict works in each of the implementation may be similar to CPython (in case of PyPy) or very different (in case of Jython). </p>
              ...
              <p> Accessing an element by index is very fast. Appending to a list is fast too. But if there is no order whatsoever, searching for a specific element is slow. We may get lucky and find an element only in a few iterations if it is near the beginning of the list. But if it is not here at all, we'll have to scan over the whole array. </p>
              <p> Tis simle list scan can be visualized as following. </p>
              <p> For example, let's say we wnat to search for 
              ...
              <p> Sure, we only have a few elements here, so scanning over them is no big deal. But what if we have a million of distinct numbers? Scanning the whole million would be slow.</p>
              <p> What we need to do is organize our data in a clever way. Here is how. Let's create a new list. You can see this list as a list of slots. Each slot will hold a number from the original list. But we'll use the number itself to compute an index of a slot. The simplest way is just take the slot <code> number % len(the_list) </code> and put our number there. Would this approach work? Not quite. For example, two numbers (TODO: compute it) would be put in the same slot. Situtations like these are called <em>collisions</em>.</p>
              <p> To make this approach viable we need to somehow <strong>resolve collisions</strong>. Let's do the following. If the slot is already occupied by some other number, we'll just check the slot right after it. And if the next slot is empty, we'll put the number there. What if the next slot is also occupied? We'll repeat the process until we finally hit an empty slot! This process of searching for an empty slot is called <strong> probing </strong>. And because we do it linearly, it is called <strong> linear probing</strong>.</p> 
